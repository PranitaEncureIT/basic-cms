...
@section('content')
  <div class="page-header">
    <h4 class="page-title">Department Links</h4>
    ...
    <div class="col-md-12">
      ...
      <div class="card">
        <div class="card-header">
            <div class="row">
                <div class="col-lg-4">
                    <div class="card-title d-inline-block">Department Links</div>
                </div>
                <div class="col-lg-3">
                    @if (!empty($langs))
                ...
                <div class="col-lg-4 offset-lg-1 mt-2 mt-lg-0">
                    <a href="#" class="btn btn-primary float-right btn-sm" data-toggle="modal" data-target="#createModal"><i class="fas fa-plus"></i> Add DLinks</a>
                </div>
            </div>
        </div>
        <div class="card-body">
          <div class="row">
            <div class="col-lg-12">
              @if (count($dlink) == 0)
                <h3 class="text-center">NO DLinks FOUND</h3>
              @else
                <div class="table-responsive">
                  <table id="dlink_section" class="table table-striped mt-3">
                    <thead>
                      <tr>
                        <th scope="col">#</th>
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLongTitle">Add DLinks</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form id="ajaxForm" class="modal-form create" action="{{route('admin.dlink.store')}}" method="POST">
            @csrf
  ...
  <div class="modal fade" id="editModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLongTitle">Edit DLinks</h5>
          <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
          </button>
        </div>
        <div class="modal-body">
          <form id="ajaxEditForm" action="{{route('admin.dlink.update')}}" method="POST">
            @csrf
            <input id="inulink_id" type="hidden" name="dlink_id" value="">
            <div class="form-group">
              <label for="">Name **</label>
...
<script>
  // let table;
  $(document).ready(function() {
      var table = $('#dlink_section').DataTable({
          processing: true,
          serverSide: true,
          paging: true,
          lengthChange: true,
      ...
      $('#dlink_section').on('click', '.deletebutton', function(e) {
...
<script>
  $(document).ready(function () {
    $(document).on("click", ".editButton", function () {
        let dlink_id = $(this).data("id");
        let name = $(this).data("name");
        let url = $(this).data("url");

        // Populate modal fields
        $("#inulink_id").val(dlink_id);
        $("#inname").val(name);
        $("#inurl").val(url);
