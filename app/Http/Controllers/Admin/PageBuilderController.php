...
class PageBuilderController extends Controller
{
    ...
    {
        $data = DB::table('sample')->orderBy('id','DESC')->get();
    ...
    public function save(Request $request)
    {
        ...
        $jsfolder = $_SERVER['DOCUMENT_ROOT'].'/cms/assets/customeJs/';
        if(!File::isDirectory($jsfolder))
            File::makeDirectory($jsfolder, 0777, true, true);
       
        $jsPath = $jsfolder.$request->id.'.js';
        if(!File::exists($jsPath)) {
            file_put_contents($jsPath, ' ');
        }

        // Replace with 'Base URL' Shortcode
        $path = url('/assets/customeJs').'/'.$request->id.'.js';
        $script = "<script type='text/javascript' src='{$path}'></script>";
        $html = str_replace(url('/'), "{base_url}", $request->html);
        $html = $script."<div class='pagebuilder-content'>" . $html . "</div>";
    ...
    public function content(Request $request)
    {
        ...
        if ($version == 'default' || $version == 'dark') {
            ...
            $testimonialSec = "<div class='testimonial-section pb-mb30 " . ($rtl == 1 ? 'pb-rtl' : '') . "'>
                <div class='container'>
                    <div class='row justify-content-center text-center'>
                        <div class='col-lg-6'>
                            <span class='section-title'>" . convertUtf8($bs->testimonial_title) . "dasfsaf</span>dfgr
            ...
            $pranitaSec = "<div class='cta-section pb-mb30 lazy " . ($rtl == 1 ? 'pb-rtl' : '') . "' style='background-image: url(" . url('assets/front/img/' . $bs->cta_bg) . "); background-size:cover;'>
                <div class='container'>
                    <div class='cta-content'>
                        <div class='row'>
                            <div class='col-md-12 col-lg-12'>
                                <table class='table table-bordered contact-btn-wrapper'>
                                    <thead><tr><th>Sr.No.</th><th>Name</th></tr></thead>
                                    <tbody><tr><td><h3 class='section-title'>".convertUtf8($bs->cta_section_text)."</h3></td><td><h3 class='section-title'>".convertUtf8($bs->cta_section_text) ."</h3></td></tr></tbody>
                                </table>
                            </div
            ...
            $marqueeSec = "<div class='cta-section pb-mb30 lazy " . ($rtl == 1 ? 'pb-rtl' : '') . "' style='background-image: url(" . url('assets/front/img/' . $bs->cta_bg) . "); background-size:cover;'>
                <div class='container'>
                    <div class='cta-content'>
                        <div class='row'>
                            <div class='col-md-12 col-lg-12'>
                                <marquee><h3 class='section-title'>".convertUtf8($bs->cta_section_text)."</h3></marquee>
                            </div                            
                        </div>
                    </div>
                </div>
        ...
        $data['pranitaSec'] = $pranitaSec;
        $data['marqueeSec'] = $marqueeSec;
