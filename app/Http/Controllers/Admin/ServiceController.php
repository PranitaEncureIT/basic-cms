...
class ServiceController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($request->hasFile('image') && $request->file('image')->isValid()) {
            ...
            $folderPath = 'cms/services/';
            $filename = CommonHelper::upload_file($image, $folderPath, '');
            if(!empty($filename)) $service->main_image = $filename; 
        }
    ...
    public function update(Request $request)
    {
        ...
        if ($request->hasFile('image') && $request->file('image')->isValid()) {
            $image = $request->file('image');
            $folderPath = 'cms/services/';
            $filename = CommonHelper::upload_file($image, $folderPath,  $service->main_image);
            if(!empty($filename)) {
    ...
    {
        $service = Service::findOrFail($request->service_id);
        @unlink('cms/services/' . $service->main_image);
